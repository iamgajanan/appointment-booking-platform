...
export async function GET(request: Request) {
  ...
  const [{ data: settings }, { data: service }, { data: hours }, { data: appointments }] = await Promise.all([
    supabase
      ...
      .select("appointment_duration_minutes, slot_interval_minutes, buffer_minutes")
  ...
  const dayHours = (hours ?? []).filter((item) => Number(item.day_of_week) === dayOfWeek);
  ...
  for (const period of dayHours) {
    ...
    for (let cursor = start; cursor + duration <= end; cursor += interval) {
      ...
      const endIso = new Date(
        `${date}T${String(Math.floor((cursor + duration + buffer) / 60)).padStart(2, "0")}:${String((cursor + duration + buffer) % 60).padStart(2, "0")}:00`,
      ).getTime();
      const overlaps = booked.some((item) => startIso < item.end && endIso > item.start);

      if (!overlaps) {
        slots.push({
          value: `${date}T${String(Math.floor(cursor / 60)).padStart(2, "0")}:${String(cursor % 60).padStart(2, "0")}:00`,
