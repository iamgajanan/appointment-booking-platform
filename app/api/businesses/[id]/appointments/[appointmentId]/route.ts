...
function getDatabaseErrorMessage(message: string) {
  if (message.includes("Appointments must be scheduled in the future")) return "Appointments must be scheduled in the future";
  if (message.includes("outside business hours")) return "The selected time is outside business hours";
  if (message.includes("duration does not match")) return "The selected time must match the service duration";
  if (message.includes("booking window")) return "The selected time is outside the allowed booking window";
  if (message.includes("appointments_no_overlapping_active") || message.includes("conflicting key value")) return "This time overlaps another appointment";
...
export async function PATCH(
  ...
  const { data: existingAppointment } = await supabase.from("appointments").select("id, status, start_at, end_at").eq("id", appointmentId).eq("business_id", id).maybeSingle();
  if (!existingAppointment) return NextResponse.json({ error: "Appointment not found" }, { status: 404 });
  ...
  if (isRescheduling && !reschedulableStatuses.has(existingAppointment.status)) return NextResponse.json({ error: "Only pending or confirmed appointments can be rescheduled" }, { status: 409 });

  if (startAt && endAt) {
    if (!isValidDate(startAt) || !isValidDate(endAt)) return NextResponse.json({ error: "Start and end times must be valid dates" }, { status: 400 });
    const startTimestamp = new Date(startAt).getTime();
    const endTimestamp = new Date(endAt).getTime();
    if (startTimestamp >= endTimestamp) return NextResponse.json({ error: "End time must be after start time" }, { status: 400 });
    if (startTimestamp <= Date.now()) return NextResponse.json({ error: "Appointments must be scheduled in the future" }, { status: 400 });

    const { data: conflict } = await supabase.from("appointments").select("id").eq("business_id", id).neq("id", appointmentId).neq("status", "cancelled").lt("start_at", new Date(endTimestamp).toISOString()).gt("end_at", new Date(startTimestamp).toISOString()).limit(1).maybeSingle();
  ...
  if (error) return NextResponse.json({ error: getDatabaseErrorMessage(error.message) }, { status: 400 });
