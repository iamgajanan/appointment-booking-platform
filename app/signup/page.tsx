...
export default function SignupPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const { data, error: signUpError } = await supabase.auth.signUp({ email, password, options: { data: { full_name: name }, emailRedirectTo: `${window.location.origin}/auth/confirm` } });
    setLoading(false);
  ...
  return (
    <main className="auth-shell"><div className="auth-card">
      ...
      <p className="auth-bottom">Already have an account? <Link href="/login">Log in</Link></p><Link href="/" className="back-link">← Back to website</Link>
    </div></main>
  );
}
